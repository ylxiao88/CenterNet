...
class PLATE(data.Dataset):
  ...
  def __init__(self, opt, split):
    ...
    self.data_dir = os.path.join(opt.data_dir, 'plate')
    #self.img_dir = os.path.join(self.data_dir, 'images')
    #_ann_name = {'train': 'trainval0712', 'val': 'test2007'}
    #self.annot_path = os.path.join(
    #  self.data_dir, 'annotations',
    #  'pascal_{}.json').format(_ann_name[split])
    self.annot_path = os.path.join(self.data_dir, 'plate_%s.json'%split)
    #self.max_objs = 50
    self.max_objs = 5
    ...
    # "train", "tvmonitor"]
    self.class_name =  ['__background__', 'plate']
